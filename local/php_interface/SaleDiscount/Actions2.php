...
class Actions2
{
	 ...
	 * @return void
	 */
	public static function applyToBasket(array &$order, $action0, $filter)
	{
if(!is_array($action0)){
			$action=$action0($order);
		}else{
			$action=$action0;
		}
//file_put_contents($_SERVER["DOCUMENT_ROOT"]."/event11.txt", "\n\n** ".date("d.m.Y H:i:s")." ****". __FILE__ ." ". __LINE__ ."****E **arFields****\n".print_r($order, true), FILE_APPEND | LOCK_EX);
file_put_contents($_SERVER["DOCUMENT_ROOT"]."/event11.txt", "\n\n** ".date("d.m.Y H:i:s")." ****". __FILE__ ." ". __LINE__ ."****E **arFields****\n".print_r($action0, true), FILE_APPEND | LOCK_EX);
//file_put_contents($_SERVER["DOCUMENT_ROOT"]."/event11.txt", "\n\n** ".date("d.m.Y H:i:s")." ****". __FILE__ ." ". __LINE__ ."****E **arFields****\n".print_r($filter, true), FILE_APPEND | LOCK_EX);
		...
		if(!empty($limitValue))
		{
			$actionDescription['ACTION_TYPE'] = \Bitrix\Sale\Discount\Formatter::TYPE_LIMIT_VALUE;
			$actionDescription['LIMIT_TYPE'] = \Bitrix\Sale\Discount\Formatter::LIMIT_MAX;
		...
		$applyBasket = array_filter($filteredBasket, '\Roztech\SaleDiscount\Actions2::filterBasketForAction');
		unset($filteredBasket);
		if (empty($applyBasket))
			return;
//file_put_contents($_SERVER["DOCUMENT_ROOT"]."/event11_1.txt", "\n\n** ".date("d.m.Y H:i:s")." ****". __FILE__ ." ". __LINE__ ."****E **arFields****\n".print_r($applyBasket, true), FILE_APPEND | LOCK_EX);
		...
		foreach ($applyBasket as $basketCode => $basketRow)
		{
			...
			if ($result >= 0)
			{
				...
				if(!empty($limitValue))
				{
					$rowActionDescription['ACTION_TYPE'] = \Bitrix\Sale\Discount\Formatter::TYPE_LIMIT_VALUE;
					$rowActionDescription['LIMIT_TYPE'] = \Bitrix\Sale\Discount\Formatter::LIMIT_MAX;
	...
	public static function applyCumulativeToBasket(array &$order, array $ranges, array $configuration = array(), $filter = null)
	{
		...
		if ($unit == self::VALUE_TYPE_FIX && $value < 0)
			$maxBound = (isset($action['MAX_BOUND']) && $action['MAX_BOUND'] == 'Y');
		$valueAction = \Bitrix\Sale\Discount\Formatter::VALUE_ACTION_CUMULATIVE;
		...
		{
			case self::VALUE_TYPE_PERCENT:
				$actionDescription['VALUE_TYPE'] = \Bitrix\Sale\Discount\Formatter::VALUE_TYPE_PERCENT;
				break;
			case self::VALUE_TYPE_FIX:
				$actionDescription['VALUE_TYPE'] = \Bitrix\Sale\Discount\Formatter::VALUE_TYPE_CURRENCY;
				$actionDescription['VALUE_UNIT'] = $currency;
				if ($maxBound)
					$actionDescription['ACTION_TYPE'] = \Bitrix\Sale\Discount\Formatter::TYPE_MAX_BOUND;
				break;
			default:
				return;
		}
	...
	public static function applyToDelivery(array &$order, array $action)
	{
		...
		if (
			...
			|| (
				...
				&& $actionDescription['VALUE_ACTION'] == \Bitrix\Sale\Discount\Formatter::VALUE_ACTION_DISCOUNT
			)
		)
			return;
	...
	{
		/*file_put_contents($_SERVER["DOCUMENT_ROOT"]."/event11_1.txt", "\n\n** ".date("d.m.Y H:i:s")." ****". __FILE__ ." ". __LINE__ ."****E **arFields****\n".print_r($action, true), FILE_APPEND | LOCK_EX);
		file_put_contents($_SERVER["DOCUMENT_ROOT"]."/event11_1.txt", "\n\n** ".date("d.m.Y H:i:s")." ****". __FILE__ ." ". __LINE__ ."****E **arFields****\n".print_r($filter, true), FILE_APPEND | LOCK_EX);
		file_put_contents($_SERVER["DOCUMENT_ROOT"]."/event11_1.txt", "\n\n** ".date("d.m.Y H:i:s")." ****". __FILE__ ." ". __LINE__ ."****E **arFields****\n".print_r($basket, true), FILE_APPEND | LOCK_EX);*/
		...
		switch (static::getUseMode())
		{
			case self::MODE_CALCULATE:
				...
				$rr=[];
				$sm=0;
foreach($result as $massa){
	if(intval($massa['PRICE'])>0){
		$rr[]=[$massa['ID'],$massa['PRICE'],$massa['QUANTITY']];
		$sm+=$massa['QUANTITY'];
	}
}	
...
$klo=floor($sm/2);
if(!empty($action['dopparam']['CNT']) && intval($action['dopparam']['CNT'])>0){
	$klo=floor($sm/$action['dopparam']['CNT']);
}
//file_put_contents($_SERVER["DOCUMENT_ROOT"]."/event11_11.txt", "\n\n** ".date("d.m.Y H:i:s")." ****". __FILE__ ." ". __LINE__ ."****E **arFields****\n".print_r($result, true), FILE_APPEND | LOCK_EX);

$chek=$klo*$action['dopparam']['CNT'];
$sumka=$klo;

$raskidat_all_sum=0;
foreach($rr as $zkl=>$rr2){
	if($klo<1){continue;}
	if($klo>=$rr2[2]){
		$klo=$klo-$rr2[2];
		foreach($result as $kluch=>$massa){
			if($massa['ID']==$rr2[0]){$result[$kluch]['OSTAVIT']=intval($rr2[2]);$raskidat_all_sum= $raskidat_all_sum + (intval($rr2[2])*$rr2[1]);}
		}	
	}else{
		foreach($result as $kluch=>$massa){
			if($massa['ID']==$rr2[0]){$result[$kluch]['OSTAVIT']=intval($klo);$raskidat_all_sum= $raskidat_all_sum + (intval($klo)*$rr2[1]);}
		}
		$klo=0;
	}
...
//file_put_contents($_SERVER["DOCUMENT_ROOT"]."/event11_11.txt", "\n\n** ".date("d.m.Y H:i:s")." ****". __FILE__ ." ". __LINE__ ."****E **arFields****\n".print_r($raskidat_all_sum, true), FILE_APPEND | LOCK_EX);
$chek0=floor($raskidat_all_sum / $chek);
...
foreach($rr as $zkl=>$rr2){
	...
	if($chek>=$rr2[2]){
		...
		$rr2['sum'] = floor($rr2[1]-(($rr2[1]*$rr2[2])-($chek0*($rr2[2]-$chek)))/$rr2[2]);
	}
	$chek=$chek-$rr2[2];
	//$rr[$zkl]['sum']=floor($chek0/$rr2[2]*$sumka);
	$rr[$zkl]=$rr2;
	foreach($result as $kluch=>$massa){
			if($massa['ID']==$rr2[0]){$result[$kluch]['SUM']=floor($rr2['sum']);}
		}
}

foreach($result as $kluch=>$massa){
		if(empty($massa['SUM'])){unset($result[$kluch]);}
	}
...
/*foreach($rr as $zkl=>$rr2){
	...
	if($klo>=$rr2[2]){
		$klo=$klo-$rr2[2];
		foreach($result as $kluch=>$massa){
			if($massa['ID']==$rr2[0]){$result[$kluch]['OSTAVIT']=intval($rr2[2]);}
		}	
	}else{
		foreach($result as $kluch=>$massa){
			if($massa['ID']==$rr2[0]){$result[$kluch]['OSTAVIT']=intval($klo);}
		}
		$klo=0;
	}
	...
	foreach($result as $kluch=>$massa){
		if(empty($massa['OSTAVIT'])){unset($result[$kluch]);}
	}*/

		//file_put_contents($_SERVER["DOCUMENT_ROOT"]."/event11_11.txt", "\n\n** ".date("d.m.Y H:i:s")." ****". __FILE__ ." ". __LINE__ ."****E **arFields****\n".print_r($rr, true), FILE_APPEND | LOCK_EX);
		//file_put_contents($_SERVER["DOCUMENT_ROOT"]."/event11_11.txt", "\n\n** ".date("d.m.Y H:i:s")." ****". __FILE__ ." ". __LINE__ ."****E **arFields****\n".print_r($result, true), FILE_APPEND | LOCK_EX);
				break;
			case self::MODE_MANUAL:
			case self::MODE_MIXED:
						...
						foreach ($basketCodeList as &$code)
						{
							...
							if (!isset($result[$code]))
							{
								if (isset($action['GIFT_TITLE']))
								{
									...
									if (
										$descr['TYPE'] == \Bitrix\Sale\Discount\Formatter::TYPE_SIMPLE
	...
	public static function setActionDescription($type, $description)
	{
		...
		if ($actionType == \Bitrix\Sale\Discount\Formatter::TYPE_SIMPLE)
			$description = (isset($description['ACTION_DESCRIPTION']) ? $description['ACTION_DESCRIPTION'] : '');

		$prepareResult = \Bitrix\Sale\Discount\Formatter::prepareRow($actionType, $description);
		unset($actionType);
	...
	public static function setActionResult($entity, array $actionResult)
	{
		...
		$prepareResult = \Bitrix\Sale\Discount\Formatter::prepareRow($actionType, $actionDescription);
	...
	{
		//file_put_contents($_SERVER["DOCUMENT_ROOT"]."/event11_11_1.txt", "\n\n** ".date("d.m.Y H:i:s")." ****". __FILE__ ." ". __LINE__ ."****E **arFields****\n".print_r($basketRow, true), FILE_APPEND | LOCK_EX);
		$value = 0.0;
		switch (static::getPercentMode())
		{
			case self::PERCENT_FROM_BASE_PRICE:
			...
			case self::PERCENT_FROM_CURRENT_PRICE:
				...
				/*if($basketRow['OSTAVIT']!=$basketRow['QUANTITY']){
					$percent0=$basketRow['OSTAVIT']*$percent/$basketRow['QUANTITY'];
					$value = ((float)$basketRow['PRICE'] * $percent0) / 100;
				}else{
					$value = ((float)$basketRow['PRICE'] * $percent) / 100;
				}*/
				$value = $basketRow['SUM']*(-1) ;
				//file_put_contents($_SERVER["DOCUMENT_ROOT"]."/event11_11.txt", "\n\n** ".date("d.m.Y H:i:s")." ****". __FILE__ ." ". __LINE__ ."****E **arFields****\n".print_r($value, true), FILE_APPEND | LOCK_EX);
	...
	protected static function compareBasketResultDescr(array $action, $resultDescr)
	{
		...
		switch ($resultDescr['TYPE'])
		{
			case \Bitrix\Sale\Discount\Formatter::TYPE_VALUE:
				...
				)
				{
					switch($action['UNIT'])
					{
						case self::VALUE_TYPE_SUMM:
							$result = (
								(
									$resultDescr['VALUE_TYPE'] == \Bitrix\Sale\Discount\Formatter::VALUE_TYPE_SUMM_BASKET
									|| $resultDescr['VALUE_TYPE'] == \Bitrix\Sale\Discount\Formatter::VALUE_TYPE_SUMM
								)
								&& $resultDescr['VALUE_UNIT'] == $currency
							);
							break;
						case self::VALUE_TYPE_PERCENT:
							$result = ($resultDescr['VALUE_TYPE'] == \Bitrix\Sale\Discount\Formatter::VALUE_TYPE_PERCENT);
							break;
						case self::VALUE_TYPE_FIX:
							$result = (
								$resultDescr['VALUE_TYPE'] == \Bitrix\Sale\Discount\Formatter::VALUE_TYPE_CURRENCY
								&& $resultDescr['VALUE_UNIT'] == $currency
							);
							break;
					}
				...
				break;
			case \Bitrix\Sale\Discount\Formatter::TYPE_MAX_BOUND:
				$result = (
					$resultDescr['VALUE'] == $value
					&& $resultDescr['VALUE_ACTION'] == $valueAction
					&& $resultDescr['VALUE_TYPE'] == \Bitrix\Sale\Discount\Formatter::VALUE_TYPE_CURRENCY

...
$strRequestUrl = "https://speller.yandex.net/services/spellservice.json/checkText?text=" . urlencode($q);
...
try {
	$curl = curl_init();
	curl_setopt($curl, CURLOPT_URL, $strRequestUrl);
...
if ($resultCheck) {
	if (count($resultCheck) == 1) {
		$listCurrentText[$resultCheck[0]["word"]] = array_values($resultCheck[0]["s"]);
	}
	elseif (count($resultCheck) > 1) {

		$arResultForMerge = [];
		foreach ($resultCheck as $arrWord) {
			$listCurrentText[$arrWord["word"]] = array_values($arrWord["s"]);
		}
	}
}

$arResult["REQUEST"]["RAW_QUERY"] = htmlspecialcharsex($q);

if (!empty($listCurrentText)) {
	foreach ($listCurrentText as $strKey => $arWord) {
		$q = str_replace($strKey , implode('||', $arWord), $q);
	}
};
...
if($this->InitComponentTemplate($templatePage))
{
	...
	if($folderPath <> '')
	{
		...
		/************* точное соотвествие - в начале  *************/

		$arPrependMatched = [];

		$arSelect = array("ID", "IBLOCK_ID", "NAME", "XML_ID", "PROPERTY_IDGLAVNOGO", "CATALOG_QUANTITY");
//		$arFilter = array("IBLOCK_ID" => 40, "?NAME" => $arResult["REQUEST"]["RAW_QUERY"] . "||" . str_replace(' ', '&&', $arResult["REQUEST"]["QUERY"]));
		$arFilter = array("IBLOCK_ID" => 40, "?NAME" => $arResult["REQUEST"]["RAW_QUERY"]);
		...
		while ($ob = $res->GetNextElement()) {
			...
			if (
				(
					(!empty($arFields['PROPERTY_IDGLAVNOGO_VALUE'])
						&& $arFields['PROPERTY_IDGLAVNOGO_VALUE'] == $arFields['XML_ID'])
					|| empty($arFields['PROPERTY_IDGLAVNOGO_VALUE'])
				)
				&& $arFields['CATALOG_AVAILABLE'] == 'Y'
			) {
				$arPrependMatched[$arFields['ID']] = $arFields['ID'];
			}

		}

		if ($_REQUEST['debug'] == 'Y' || $_SESSION["searchingDebug"] == 'Y') {
			echo "<font color=green>Запрос: " . $arFilter["?NAME"] . "&nbsp;&nbsp;&nbsp;</font>";

			$_SESSION["searchingDebug"] = 'Y';
		}

		if ($_REQUEST['debug'] == 'N') {
			$_SESSION["searchingDebug"] = 'N';
		}

		/************* оставляем только главный размер *************/

		$arSelect = array("ID", "IBLOCK_ID", "NAME", "XML_ID", "PROPERTY_IDGLAVNOGO", "CATALOG_QUANTITY");
		$arFilter = array("IBLOCK_ID" => 40, "ID" => $arReturn);
		$res = CIBlockElement::GetList(array(), $arFilter, false, false, $arSelect);
		while ($ob = $res->GetNextElement()) {
			$arFields = $ob->GetFields();

			if (!empty($arFields['PROPERTY_IDGLAVNOGO_VALUE'])
				&& $arFields['PROPERTY_IDGLAVNOGO_VALUE'] != $arFields['XML_ID']) {
				$arNeedToDelete[$arFields['ID']] = $arFields['ID'];
			}

			if ($arFields['CATALOG_AVAILABLE'] == 'N')
				$arNeedToDelete[$arFields['ID']] = $arFields['ID'];

		}

		foreach ($arPrependMatched as $key => $item) {
			if (in_array($item, $arNeedToDelete)) {
				unset($arPrependMatched[$key]);
			}
		}

		foreach ($arReturn as $key => $item) {
			if (in_array($item, $arNeedToDelete)) {
				unset($arReturn[$key]);
			}
		}
		...
		if ($_REQUEST['debug'] == 'Y' || $_SESSION["searchingDebug"] == 'Y') {
			echo "<font color=green>По заголовкам: " . count($arPrependMatched) . "&nbsp;&nbsp;&nbsp;</font>";
			echo "<font color=green>По тексту: " . count($arReturn) . "&nbsp;&nbsp;&nbsp;</font>";
		}

		if (!empty($arPrependMatched)) {

			if (count($arPrependMatched) > 15)
				$arReturn = array_values($arPrependMatched);
			else
				$arReturn = array_values($arPrependMatched) + array_values($arReturn);

			$arReturn = array_unique($arReturn);
		}

		foreach ($arResult["SEARCH"] as $key => $item) {
			if (in_array($item['ITEM_ID'], $arNeedToDelete)) {

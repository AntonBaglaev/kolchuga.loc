...
if($this->StartResultCache(false, ($arParams["CACHE_GROUPS"]? $USER->GetGroups(): false)))
{
	...
	if(!empty($arResult["ITEMS"]))
	{
		if ($this->facet->isValid())
		{
			...
			$extFilter = $GLOBALS[$arParams['FILTER_NAME']];
			
			if(is_array($extFilter)) $arResult['FACET_FILTER'] = array_merge($arResult['FACET_FILTER'], $extFilter);
		...
		else
		{
			...
			$extFilter = $GLOBALS[$arParams['FILTER_NAME']];
			if(is_array($extFilter)) $arElementFilter = array_merge($arElementFilter, $extFilter);	

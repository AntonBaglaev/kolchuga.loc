...
//\Kolchuga\Settings::xmp($arResult['DISPLAY_PROPERTIES'],11460, __FILE__.": ".__LINE__);
?>
			...
			<div class="price-block">
				...
				<!--<div class="buy-block">
					<noindex>
						...
						if ($arResult['CATALOG_QUANTITY']==1 || $arResult['SKU_COUNT']==1) {?>
							<noindex><a class="btn btn-primary popup-modal order-btn" href="#modal-buyoneclick" rel="nofollow" rel_mc="rezerv">Зарезервировать</a></noindex>
						...
						<?}elseif(!$arResult['PROPERTIES']['TREBUETSYA_LITSENZIYA']['VALUE']){?>
						...
						<?}else{?>
							...
							<div class="catalog__license">
								<div class="catalog__license__item">
									<span><br/><a href="/information/rezerv-grazhdanskogo-oruzhiya-i-patronov.php" class="rules"><ins>Правила резервирования</ins></a></span>
								</div>
							</div>
				...
				<div class="catalog__btn text-center buy-block">
            <noindex>
			<?/*<div style="padding: 10px 0;color:#ff0000;">Внимание!<br> На сайте ведутся технические работы.<br> Уточняйте информацию о наличии товара и ценах у менеджеров салонов.</div>*/?>
				...
				//if($arResult['IBLOCK_SECTION_ID']!=18130){ //podarochni_sertifikat
						...
						if(!$arResult['PROPERTIES']['TREBUETSYA_LITSENZIYA']['VALUE']){?>
							<div class="alert-size">
								...
								<?if(1==2){?><noindex><strong>Временно оформление заказа в интернет-магазине недоступно.</strong></noindex><?}else{?>
									<a class="js-buy-btn btn btn-primary popup-modal2 order-btn" href="#modal-komplekt" style="<?=$arResult['CAN_BUY'] ? '' : 'display: none'?>" data-id="<?=$buy_id?>" rel_mc="kupit" rel="nofollow">Купить</a>
								...
								<?else:?>
									...
									<?}else{?>
										...
										<?/* $only_luberci='Y';
										foreach($arResult['SKU_SIZES_IN_STORES'] as $storeID=>$store){
											if($storeID!=116 && $store['STORE']['AMOUNT']>0){$only_luberci='N';}
										}
										$yes_lenin='Y';
										if(!empty($arResult['SKU_SIZES_IN_STORES'][114])){
											foreach($arResult['SKU_SIZES_IN_STORES'] as $storeID=>$store){
												if($storeID!=116 && $storeID!=114 && $store['STORE']['AMOUNT']>0){$yes_lenin='N';}											
											}
										}else{
											$yes_lenin='N';
										}
										?>
										<?if($only_luberci=='N' && $yes_lenin=='N'){?>
											<a class="js-buy-btn btn btn-primary order-btn" style="<?=$arResult['CAN_BUY'] ? '' : 'display: none'?>" href="#" data-id="<?=$buy_id?>" rel="nofollow" rel_mc="kupit">Купить</a>
										<?}else{?>
											<?if($yes_lenin=='Y'){?>
												<noindex><a href="/weapons_salons/leninsky-prospekt/"><strong>Покупка товара только в салоне</strong></a></noindex>
											<?}else{?>
												<noindex><a href="/weapons_salons/lyubertsy/"><strong>Покупка товара только в салоне</strong></a></noindex>
											<?}?>
										<?} */?>
										<?
										if($buy_id>0){
											foreach($arResult['LIST_DOSTUPNOST_ITEM'] as $key=>$valueI){
												if($valueI['ID']==$buy_id){
													if(!empty($valueI['GET_BY_SKLAD'])){
														?><a class="order-btn js-buy-btn btn btn-primary " style="<?=$arResult['CAN_BUY'] ? '' : 'display: none'?>" href="#" data-id="<?=$buy_id?>" rel="nofollow" rel_mc="kupit">Купить</a><?													
													}else{
						...
						<?}else{?>
							...
							<div class="catalog__license">
								<div class="catalog__license__item">
									<span><span class="dopkrezerv">Только по документам</span><br/><a href="/information/rezerv-grazhdanskogo-oruzhiya-i-patronov.php" class="rules"><ins>Правила резервирования</ins></a></span>
								</div>
							</div>

...
//в этом шаблоне не актуально это условие
/*$arPageSizes = array(
...
$arResult['BRAND_VALUE']=$arResult['PROPERTIES']['BREND'];
$arResult['KALIBR_VALUE']=$arResult['PROPERTIES']['KALIBR'];
$cp = $this->__component; // объект компонента
if (is_object($cp)){
	
   $cp->SetResultCacheKeys(array('TIMESTAMP_X','~NAME','BRAND_VALUE','KALIBR_VALUE'));
}
		...
		<div class="detail-desc">
				...
				<!--<div class="buy-block catalog__btn text-center">
					<noindex>
						...
						<?}else{?>
							...
							<?else:?>
								<noindex><a class="btn btn-primary order-btn" href="#" aria-disabled="true" disabled rel="nofollow">Нет в наличии</a></noindex>
							<?endif;?>
					...
					<?endif;?>
					 <div class="link_uncertainty">
						<a class="popup-modal rules" href="#modal-sizeuncertainty">Не уверены в размере?</a>
					</div>
				</div>-->
				<div class="buy-block catalog__btn text-center">
            <noindex>
			<?/*<div style="padding: 10px 0;color:#ff0000;">Внимание!<br> На сайте ведутся технические работы.<br> Уточняйте информацию о наличии товара и ценах у менеджеров салонов.</div>*/?>
				...
				//if($arResult['IBLOCK_SECTION_ID']!=18130){ //podarochni_sertifikat
						...
						<?}else{?>
							<?if($arResult["CATALOG_QUANTITY"]>0):?>
								<noindex><a class="order-btn btn btn-primary popup-modal" href="#modal-buyoneclick" rel="nofollow" rel_mc="rezerv">Зарезервировать</a></noindex>
                ...
                <div class="size_list">
                    <table>
                        <tr class="size_list_thead">
                            <td class="in_store">В наличии</td>
        ...
        <div class="catalog__option-list">
            ...
            if($arResult['SKU'] && $arResult['SKU_COUNT'] > 0):?>
                <div class="catalog__option-list__item">
                    <select name="SKU_SIZE"<?=$arResult['SKU_COUNT'] > 1 ? '' : ' disabled'?>>
                        <?if($arResult['SKU_COUNT'] > 1):?>
                            <option value="">Выбрать размер 111</option>
                        <?endif;
                        foreach(reset($arResult['SKU']) as $code => $item):?>
                            <option value="<?= $item['ID']?>"
                                    data-price="<?=$item['MIN_PRICE']['PRINT_VALUE']?>"

...
if(empty($arResult['SECTIONS']) && $arResult['SECTION']['IBLOCK_SECTION_ID'] > 0){
    ...
    $res = \CIBlockSection::GetList(
        ...
        false,
        array('ID', 'IBLOCK_ID', 'SECTION_PAGE_URL', 'NAME','DEPTH_LEVEL')
    );
    ...
    while($section = $res->GetNext()){
        ...
        if($section['ID'] == $arResult['SECTION']['ID']){
            ...
            $section['RELATIVE_DEPTH_LEVEL'] = $section['DEPTH_LEVEL'];

        $arResult['SECTIONS'][] = $section;
    }
	$arResult['SECTIONS_COUNT'] = count($arResult['SECTIONS']);
}
//if ($USER->GetID()=="11460"){echo "<pre>";print_r($arResult);echo "</pre>";}
?>
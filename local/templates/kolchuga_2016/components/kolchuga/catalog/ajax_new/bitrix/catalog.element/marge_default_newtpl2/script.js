...
$(document).ready(function () {
	...
	size_list_table.on('click', function (e) {
		...
		if (v > 0 && $(this).data('max') > 0) {
			...
			if(by=='Y'){
				$('.catalog__btn noindex').html('<a class="order-btn js-buy-btn btn btn-primary " href="#" data-id="'+v+'" rel="nofollow" rel_mc="kupit">Купить</a>');
			}else if(by=='LENIN'){
				$('.catalog__btn noindex').html('<a href="/weapons_salons/leninsky-prospekt/"><strong>Покупка товара только в салоне</strong></a>');
			}else if(by=='LUBER'){
				$('.catalog__btn noindex').html('<a href="/weapons_salons/lyubertsy/"><strong>Покупка товара только в салоне</strong></a>');
			}else if(by=='BARVIHA'){
				$('.catalog__btn noindex').html('<a href="/news/open_barvikha/"><strong>Покупка товара только в салоне</strong></a>');
			}
			btn = $('.js-buy-btn');

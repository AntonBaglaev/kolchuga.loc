...
/* \Kolchuga\Settings::xmp($arResult['SECTION']['SECTION_PAGE_URL'].$arResult['CODE'].'/',11460, __FILE__.": ".__LINE__);
\Kolchuga\Settings::xmp($arResult['DETAIL_PAGE_URL'],11460, __FILE__.": ".__LINE__);
\Kolchuga\Settings::xmp($arResult['ORIGINAL_PARAMETERS']['CURRENT_BASE_PAGE'],11460, __FILE__.": ".__LINE__);
\Kolchuga\Settings::xmp($arResult['SECTION'],11460, __FILE__.": ".__LINE__);
...
$arResult['SETKA']=[];
	if(!empty($arResult['PROPERTIES']['RAZMER']['VALUE'])){
		$rs = \CIBlockElement::GetList (
			Array(),
			Array("IBLOCK_ID" => 67, "NAME" => $arResult['PROPERTIES']['BREND']['VALUE'], 'ACTIVE'=>'Y'),
			false,
			Array (),
			Array("ID", "IBLOCK_ID", "NAME", "DETAIL_TEXT", 'PROPERTY_FILTER_PROP', 'PROPERTY_FILTER_SECTION')
		);
		$sectar=[];
		$sectar[]=$arResult['SECTION']['ID'];
		foreach($arResult['SECTION']['PATH'] as $vvl){
			$sectar[]=$vvl['ID'];
		}
		while($ar_fields = $rs->GetNext()) {
			if(!empty($ar_fields['PROPERTY_FILTER_PROP_VALUE']) || !empty($ar_fields['PROPERTY_FILTER_SECTION_VALUE'])){
				...
				//\Kolchuga\Settings::xmp($ar_fields['PROPERTY_FILTER_SECTION_VALUE'],11460, __FILE__.": ".__LINE__);
				//\Kolchuga\Settings::xmp($sectar,11460, __FILE__.": ".__LINE__);
				$filtersetka=["IBLOCK_ID" => $arResult['IBLOCK_ID'], 'ID'=>$arResult['ID']];

...
?><!--pre><?print_r($GLOBALS[$arParams["FILTER_NAME"]]['PROPERTY_CSECTIONS']);?></pre--><?
?><!--pre><?print_r($arResult);?></pre--><?*/
//добавляем проверку на наличие товаров бренда, а то разделы пустые местами вылезают
$filya=array(
   "IBLOCK_ID" => 40, 
	   'PROPERTY_BREND_VALUE'=>[],
		'SECTION_ID' => $GLOBALS[$arParams["FILTER_NAME"]]['PROPERTY_CSECTIONS'],
...
foreach($arResult["ITEMS"] as $arItem){
	$filya['PROPERTY_BREND_VALUE']=$arItem['PROPERTY_FILTER_DESCRIPTION'];
...
$rs = \CIBlockElement::GetList(
   ...
   false, 
   Array("nTopCount"=>10),
   array("ID",'PROPERTY_BREND')
);
while($ar = $rs->Fetch()) {
$nalichtovar[$ar['PROPERTY_BREND_VALUE']][]=$ar['ID'];
}
}
foreach($arResult["ITEMS"] as $key=>$arItem)
    {
		$ost='N';
		foreach($arItem['PROPERTY_FILTER_DESCRIPTION'] as $br){
			if(!empty($nalichtovar[$br])){$ost='Y';}		
		}
		if($ost=='N'){unset($arResult["ITEMS"][$key]);}
	}
  ...
  $arResult['TAG_GROUP'] = array();
  $arResult['TAG_GROUP_REV'] = array(0 => 'Общие');
  ...
  while($dataProp = $objProp->GetNext()){
    $arResult['TAG_GROUP'][$dataProp['VALUE']] = $dataProp['XML_ID'];
    $arResult['TAG_GROUP_REV'][$dataProp['XML_ID']] = $dataProp['VALUE'];
  }
      ...
      !empty($arResult["ITEMS"]))
  {
    ...
    {		
      if (intval($arItem["IBLOCK_SECTION_ID"]) > 0)
      {
        ...
        //группировка
        if($arItem['PROPERTY_TAG_GROUP_VALUE'] == NULL){
          $secGroup[intval($arItem["IBLOCK_SECTION_ID"])]["ITEMS"][0][] = &$arItem;
        }else{
          $secGroup[intval($arItem["IBLOCK_SECTION_ID"])]["ITEMS"][$arResult['TAG_GROUP'][$arItem['PROPERTY_TAG_GROUP_VALUE']]][] = &$arItem;
        }

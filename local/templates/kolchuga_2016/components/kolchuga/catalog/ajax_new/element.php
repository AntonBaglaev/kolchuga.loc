...
$allArray = explode('/', $curDir);

$ooo=2;
if($ooo==1){
 $NewTpl = '.default';
if(($allArray[2]!='odezhda')and($allArray[2]!='obuv_3')and($allArray[3]!='odezhda_2')and($allArray[3]!='obuv_4')and($allArray[2]!='podborki')){
    $NewTpl = 'newTpl';
}
if(($allArray[3]=='golovnye_ubory')||($allArray[3]=='dopolnitelnye_aksessuary')){
	$NewTpl = 'newTpl';
}
}else{
//marge_default_newtpl - объединил оба шаблона, надоело пытаться понять чего где не хватает.27,05,21
$NewTpl = 'marge_default_newtpl2';
}
if(!empty($_REQUEST['defa_old'])){	
	if($_REQUEST['defa_old']=='Y'){$NewTpl = 'defa_old';}
	else{$NewTpl = $_REQUEST['defa_old'];}
}
$asidevenu='left';
//if($USER->GetID() ==11460){
	$asidevenu='left4';
//}
/* ?><!--pre>Tpl:<?=$NewTpl?> <?print_r($allArray);?></pre--><? */
?>
<div class="catalog">
...
<!--<div class="sTabs">
  ...
  <?/* <h2><?=GetMessage("CT_TAB_VIEWED");?></h2>		 */				?>					
  ...
  <div class="description-block">
   <?
	  ...
	  $sort_f=$arParams["TOP_ELEMENT_SORT_FIELD"];
	  $sort_o=$arParams["TOP_ELEMENT_SORT_ORDER"];
	  if($GLOBALS["DelElementSimilar"]['RAZDEL'] == 17827){
		$sort_f='CATALOG_PRICE_SCALE_2';
		$sort_o='desc';
	  }
	  /*?><!--pre>DelElementSimilar <?print_r($GLOBALS["DelElementSimilar"]);?></pre--><?*/
   ?>
	<h2>Похожие товары<?//=GetMessage("CT_TAB_SIMILAR")?></h2>																	  
            <?$APPLICATION->IncludeComponent(
              "bitrix:catalog.top",
              "",
              array(

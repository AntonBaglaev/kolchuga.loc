...
<div class="container-fluid pb-5">
            ...
            <div class="rowanons" id="<?= $this->GetEditAreaId($osnovnoy['ID']); ?>">
                <div class="row">
                    <div class="col-md-6 order-2">
                        ...
                        <div class="osnanons">
                            <? if ($osnovnoy["PREVIEW_TEXT"]) {
                                echo $osnovnoy["PREVIEW_TEXT"];
                            } ?>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 order-1 order-md-2">
                        <?
                        if (!empty($osnovnoy['PROPERTIES']['VIDEO_YOUTUBE']['VALUE'])) {
                            ?>
                            <div class="thumb-wrap">
                                <iframe width="560" height="315"
                                        src="<?= $osnovnoy['PROPERTIES']['VIDEO_YOUTUBE']['VALUE'] ?>" frameborder="0"
                                        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                        allowfullscreen></iframe>
                            </div>
                            <?
                        } elseif ($arParams["DISPLAY_PICTURE"] != "N" && !empty($arResult["DOPOLNITELNO"][$osnovnoy["ID"]]['DETAIL_PICTURE_SRC'])) {
                            ?>
						...
						<?else:*/
                            ...
                            <img class="preview_picture " border="0"
                                 src="<?= $arResult["DOPOLNITELNO"][$osnovnoy["ID"]]['DETAIL_PICTURE_SRC'] ?>"
                ...
                <div class="row">
                    <div class="col-md-8 osntext pr-md-5">
                        <? if ($osnovnoy["DETAIL_TEXT"]) {
                            echo $osnovnoy["DETAIL_TEXT"];
                        } ?>
                        <? if (!empty($osnovnoy['DISPLAY_PROPERTIES']['NEXT_SALON']['VALUE'])) { ?>
                            ...
                            <div class="next_salon">
                                <div class="">
                                    ...
                                    <script>console.log(<?echo json_encode($osnovnoy['DISPLAY_PROPERTIES']['NEXT_SALON'])?>);</script>
                                    <? foreach ($osnovnoy['DISPLAY_PROPERTIES']['NEXT_SALON']['VALUE'] as $id) { ?>
                                        <div class='next_salon_item'>
                                            <? if ($id == 699777) { ?>
                                                ...
                                                Сервисный центр <?= ($osnovnoy['DISPLAY_PROPERTIES']['NEXT_SALON']['DOPOLNITELNO'][$id]['IN'] == 'MSK' ? 'на' : 'в') ?> <?= $osnovnoy['DISPLAY_PROPERTIES']['NEXT_SALON']['DOPOLNITELNO'][$id]['PROPERTY_ADDRESS_SRC'] ?>
                                                <br>
                                                <?
                                            } ?>
                                            <?= $osnovnoy['DISPLAY_PROPERTIES']['NEXT_SALON']['DOPOLNITELNO'][$id]['PROPERTY_PHONES_SERVICE_VALUE'] ?>
                                            <br>
                                            Часы
                                ...
                                <div class="recomend_list">
                                    <ul>
                                        <? foreach ($osnovnoy['DISPLAY_PROPERTIES']['REKOMEND']['LINK_ELEMENT_VALUE'] as $idR => $vlR) { ?>
                                            <li><a href="<?= $vlR['DETAIL_PAGE_URL'] ?>"><?= $vlR['NAME'] ?>
                                                    >></a><br><span><?= $osnovnoy['DISPLAY_PROPERTIES']['REKOMEND']["DOPOLNITELNO"][$idR]['DATA_FORMAT'] ?></span>
                                            </li>
                                            <?
                                        } ?>
                                    </ul>
    ...
    <div class="row list_service_container news_list_wrapper">
        ...
        <? foreach ($arResult["ITEMS"] as $arItem): ?>
            ...
            <? if (empty($arResult["DOPOLNITELNO"][$arItem["ID"]]['PROPERTY_ANONS_VALUE']) || empty($arItem["PREVIEW_PICTURE"])) {
                continue;
            } ?>
            <?
            ...
            <div class="col-md-6 service_item news_list-item" id="<?= $this->GetEditAreaId($arItem['ID']) ?>">
                <div class="row g-0 overflow-hidden flex-md-row news_list-row">
                    ...
                    <div class="col d-flex flex-column position-static news_list-description">
                        ...
                        <p class="card-text mb-auto news_list-subtext"><?= $arResult["DOPOLNITELNO"][$arItem["ID"]]['PROPERTY_ANONS_VALUE']['TEXT'] ?: '' ?></p>
				...
				<div class="col-md-6">
					...
					<div class="container-fluid h-100">
						<div class="row h-100">
							...
							<?endif;?>
							<br>
							<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class='btn-blue mx-0'>Подробнее</a>
							</div>

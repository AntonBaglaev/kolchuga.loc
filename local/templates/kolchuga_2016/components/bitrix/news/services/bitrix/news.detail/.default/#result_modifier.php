...
if(!empty($arResult['DISPLAY_PROPERTIES']['TOVARS_IN_NEWS']['VALUE'])){
		...
		$dbEl = \CIBlockElement::GetList(array('sort' => 'asc'), array('IBLOCK_ID' => 40, 'ID'=>$arResult['DISPLAY_PROPERTIES']['TOVARS_IN_NEWS']['VALUE'], ), false, false, Array('ID','CATALOG_TYPE','DETAIL_PICTURE','PROPERTY_OLD_PRICE','CATALOG_GROUP_2','DETAIL_PAGE_URL','NAME','ACTIVE', 'XML_ID','PROPERTY_IDGLAVNOGO'));
		//$res->SetUrlTemplates("#SITE_DIR#/internet_shop/#SECTION_CODE_PATH#/#ELEMENT_CODE#/");
		$arResult['DISPLAY_PROPERTIES']['TOVARS_IN_NEWS']['SKU2']=[];	
		$arXmlIDGl=[];
		while($arEl = $dbEl->GetNext())	
		{				
			...
			$arXmlIDGl[$arEl['ID']]=$arEl['XML_ID'];

			$arResult['DISPLAY_PROPERTIES']['TOVARS_IN_NEWS']["DOPOLNITELNO"][$arEl['ID']]=$arEl;
		} */
		$elget=\Kolchuga\DopViborka::getItemInDopolnitelno(
			$param=array('IBLOCK_ID'=>40, 'TYPE_PRICE'=>array('Розничная'), 'SELECT'=>Array('ID','CATALOG_TYPE','DETAIL_PICTURE','PROPERTY_OLD_PRICE','CATALOG_GROUP_2','DETAIL_PAGE_URL','NAME','ACTIVE', 'XML_ID','PROPERTY_IDGLAVNOGO')),

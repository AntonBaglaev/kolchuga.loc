...
if (isset($_POST[$action_var]) && strlen($_POST[$action_var]) > 0)
{
	...
	else if ($_POST[$action_var] == "recalculate")
	{
        ...
        $arIDSinBasket = array();
        foreach ($arRes["BASKET_DATA"]['ITEMS']['AnDelCanBuy'] as $key => $arItem) {
                        ...
                        if($ar_fields["PROPERTY_IS_SKU_VALUE"]=="Нет"){
                            $glavnyTovar = array(
                                ...
                                "KOMPLEKT" => array(
                                    ...
                                    "DETAIL_PICTURE" => CFile::ResizeImageGet($ar_fields["DETAIL_PICTURE"], array('width' => 200, 'height' => 200), BX_RESIZE_IMAGE_PROPORTIONAL)
                                ),
                            );
                        }
                        $ids[] = $ar_fields["ID"];

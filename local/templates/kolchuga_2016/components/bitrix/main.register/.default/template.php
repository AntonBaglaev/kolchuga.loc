...
if (count($arResult["ERRORS"]) > 0):
	foreach ($arResult["ERRORS"] as $key => $error){
		if ($key === "LOGIN") continue;
			$issetLogin = strpos($error, $arResult["VALUES"]["EMAIL"]);
		/*echo $arResult["VALUES"]["EMAIL"];
		echo $key;
		echo $issetLogin;*/
		if ($_GET['dev'] == 1) {
			/*echo "<pre>";
		...
		if ($arResult["VALUES"]["EMAIL"] && $key === 0 && $issetLogin != "") {?>
			<div class="error-msg"><div class="alert alert-danger">Пользователь с таким e-mail (<?=$arResult["VALUES"]["EMAIL"]?>) уже существует. <br> <a href="/personal/profile/?forgot_password=yes">Восстановить пароль</a></div></div>
		<?
		} else {
			if ($arResult["ERRORS"][$key]){
				$arResult["ERRORS"][$key] = str_replace("#FIELD_NAME#", "&quot;".GetMessage("REGISTER_FIELD_".$key)."&quot;", $error);?>
				<div class="error-msg"><div class="alert alert-danger"><?=$arResult["ERRORS"][$key]?></div></div>
			<?}
		}
	}
...
/* RESORT*/
?>
<input type="hidden" class="js-login" name="REGISTER[LOGIN]" value="">
		...
		default:
			...
			?><div class="form__input">
				<input size="30"
					...
					<?if($FIELD == 'EMAIL'):?>onchange="$('.js-login').val($(this).val())"<?endif?>
			/><?
...
if ($arResult["USE_CAPTCHA"] == "Y")
{
	...
	?><div class="form__group form__group--captcha">
		...
		<div class="form__input">
			...
			<span class="form__input__img form__input__captcha">
				<img src="/bitrix/tools/captcha_reg.php?captcha_sid=<?=$arResult["CAPTCHA_CODE"]?>" alt="CAPTCHA" />
			</span>
...
<div class="form__group">
	<label class="label--checkbox">
		<p class="form__group_regis"><input type="checkbox" name="SET_SUBSCRIBE" value="1" checked="checked">
				Подписка на рассылку новостей</p>
	</label>
</div>

<div class="form__group">
	<label class="label--checkbox">
		<p class="form__group_regis"><input type="checkbox" name="REGISTER[AGREE]" value="1" checked="checked">

...
<ul class="nav navbar__nav">
	...
	foreach($arResult as $arItem):
		...
		<li class="aim_parent <?=$arItem['PARAMS']['CLASS']?> lvl_<?=$arItem["DEPTH_LEVEL"]?>" data-submenu-id="<?=$id?>">
			...
			<ul id="<?=$id?>">
		<?else:?>
			<li class="<?=(!empty($arItem['PARAMS']['CLASS'])? $arItem['PARAMS']['CLASS'].' ':'')?>lvl_<?=$arItem["DEPTH_LEVEL"]?>" data-submenu-id="<?=$id?>"><a href="<?=$arItem['LINK']?>" class="<?=$arItem['SELECTED'] ? 'active' : ''?>" title="<?=$arItem['TEXT']?>"><?=$arItem['TEXT']?></a></li>
		<?endif;

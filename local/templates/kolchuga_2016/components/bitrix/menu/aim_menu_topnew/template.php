...
<ul class="nav navbar__nav justify-content-between aim_menu_topn">
	<?$prev_lv = 0;
	foreach($arResult as $arItem):
		...
		<li class="aim_parent <?=$arItem['PARAMS']['CLASS']?> lvl_<?=$arItem["DEPTH_LEVEL"]?>" data-submenu-id="<?=$id?>">
			<a href="<?=$arItem['LINK']?>" class="<?=$arItem['SELECTED'] ? 'active' : ''?>" title="<?=$arItem['TEXT']?>"><?=$arItem['TEXT']?></a>
			<ul id="<?=$id?>">
		<?else:?>
			<li class="lvl_<?=$arItem["DEPTH_LEVEL"]?>" data-submenu-id="<?=$id?>"><a href="<?=$arItem['LINK']?>" class="<?=$arItem['SELECTED'] ? 'active' : ''?>" title="<?=$arItem['TEXT']?>"><?=$arItem['TEXT']?></a></li>
		<?endif;

		...
		<div class="row" id="<?=$this->GetEditAreaId($arResult['ID']);?>">
			...
			<div class="col-md-6 order-1 order-md-2">
			 <?if($arParams["DISPLAY_PICTURE"]!="N" && !empty($arResult["DETAIL_PICTURE"])):?>
						...
						<?else:*/?>
							<img class="preview_picture " border="0" src="<?=$arResult["DETAIL_PICTURE"]['SRC']?>" width="100%"  alt="<?=$arItem["NAME"]?>" title="<?=$arItem["NAME"]?>" />
						<?//endif;?>
						...
						<div class="row">							
							<?foreach($arResult["GALLERY"] as $k => $arImage){?>
								<div class="col-md-3 col-sm-4 col-6 middle">
									<a data-gallery="gallery-news-<?=$arResult["ID"]?>" href="<?=$arImage["SRC_LG"]?>" title="<?=$arImage["DESC"]?>" class="cursor-loop fancybox" data-fancybox-group="gallery-news-<?=$arResult["ID"]?>">
...
<div class="next_salon">
			...
			<?foreach($arResult['DISPLAY_PROPERTIES']['NEXT_SALON']['VALUE'] as $id){?>
				...
				<?=$arResult['DISPLAY_PROPERTIES']['NEXT_SALON']['DOPOLNITELNO'][$id]['PROPERTY_PHONES_SERVICE_VALUE']?><br>
				...
				<?if (!empty($arResult['DISPLAY_PROPERTIES']['NEXT_SALON']['DOPOLNITELNO'][$id]['PROPERTY_DOP_DESCRIPTION_VALUE'])){
						?>
						<br><?=$arResult['DISPLAY_PROPERTIES']['NEXT_SALON']['DOPOLNITELNO'][$id]['PROPERTY_DOP_DESCRIPTION_VALUE']['TEXT']?>
						<?
					}?>
				</div>
			<?}?>

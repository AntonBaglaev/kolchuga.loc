	...
	<table class="table table__info">
		...
		<tr>
			...
			<td>
				...
				<span><?=$arResult['STATUS']['DESCRIPTION']?></span>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<th>Действия</th>
			<td>
				<?$payment = $arResult['PAYMENT'][0];
				if($payment["PAID"] !== "Y" && $payment["CAN_REPAY"]=="Y" &&
						$payment["PAY_SYSTEM"]["PSA_NEW_WINDOW"] == "Y"):?>
				<a href="<?=$payment["PAY_SYSTEM"]["PSA_ACTION_FILE"]?>" target="_blank" title="Оплатить" class="btn btn-primary"><span>Оплатить</span></a>
				<?endif?>
				<?if($arResult["CANCELED"] == "Y" || $arResult["CAN_CANCEL"] == "Y"):?>
					<?if($arResult["CANCELED"] == "Y"):?>
					...
					<td class="td__price">
						...
						<div class="td__inner">
							<div class="price__block">
								<span class="price">
									<?if($arResult['HAS_DISCOUNT']) echo
										number_format($prod['PRICE'] + $prod['DISCOUNT_PRICE'], 2, '.', ' ') . ' руб.'?></span>
							</div>
						</div>
					</td>
					...
					<td class="td__sum">
						...
						<div class="td__inner">
							<span class="price-total">
								<?=number_format(($prod['PRICE']) * $prod['QUANTITY'], 2, '.', ' ') . ' руб.'?>
							</span>
						</div>
					</td>
				</tr>
	...
	<div class="text-right">
		<div class="table__total--wrap">
			<table class="table table__total">
				...
				<?//$res = CIBlockElement::GetList(array(), arra, array(), false, array('ID', 'IBLOCK_ID'));?>
				<? ///// DISCOUNT ?>

...
/*	==========================================================================
	...
	function resizeNewsGrid() {
		var newsGridCol = $('.banner-grid__col_banner');
		var newsGridColLeft = $('.banner-grid__col_left').find('img');
		if (newsGridColLeft.height() === 0 && newsGridColLeft.length) {
			newsGridColLeft.load(function(){
	    		if (newsGridCol.length && $(window).width() >= 446) {
	    			newsGridCol.height(newsGridColLeft.height());
	    		} else {
	    			newsGridCol.height('auto');
	    		}
			});
		} else {
			if (newsGridCol.length && $(window).width() >= 446) {
	...
	setTimeout(resizeNewsGrid, 1000);
	$(window).resize(function(){
		resizeNewsGrid();
	});

	BX.showWait = function () {
		var loader = '<div class="loader"><div class="lds-roller">'+('<div></div>'.repeat(8))+'</div></div>';
		$('body').append(loader);
	};

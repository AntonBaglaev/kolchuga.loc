...
<script type="text/javascript">
	jQuery(function() {
		...
		$.fn.startCarousel = function() {
			var bodywidth = $('#bigdata_carousel').width(),
				itemwidth = $('.bigdata_gallery ul li').outerWidth(true),		
				mycontwidth = bodywidth > itemwidth ? bodywidth - bodywidth%itemwidth : itemwidth,
				licount = $('.bigdata_gallery ul li').size(),
				jscroll = 1;
			...
			$('.bigdata_carousel_content').width(mycontwidth);
				
			$('.bigdata_gallery ul').jcarousel({
				scroll:jscroll,
				wrap:'circular'
			});
		};
...
<h1 class="border_circl color_blue">Рекомендуем</h1>
<div class="bigdata_gallery" id="bigdata_carousel">
    <div class="bigdata_carousel_content">
        <ul>
			<?$i = 0;
			...
			?>
			<li>
				<div class="bigdata_img">
					<a href="<?= $arItem['DETAIL_PAGE_URL']?>?id=<?=$arItem['ID']?>&rcm=<?=base64_encode($arItem['ADD_URL'])?>">
						<!-- resize img 200x200 -->
						<img src="<?=$arItem['PREVIEW_PICTURE']['SRC']?>" alt="<?=$arItem['NAME']?>"/>
					</a>
				</div>
				<div class="bigdata_title"><a href="<?=$arItem['DETAIL_PAGE_URL']?>?id=<?=$arItem['ID']?>&rcm=<?=base64_encode($arItem['ADD_URL'])?>"><?=$arItem['NAME']?></a></div>
				...
				<div class="button_buy">
					<div class="button_blue plus">
						<?if(!$arItem['TP']):?>
						<a class="js-buy-btn" rel="nofollow" href="#" data-id="<?=$arItem['ID']?>">Купить</a>
						<?else:?>
							<a href="<?=$arItem['DETAIL_PAGE_URL']?>">Купить</a>
						<?endif?>
					</div>
				</div>
				<?*/?>

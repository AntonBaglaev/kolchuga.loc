...
<?else:?>
	<input type="text" class="input_inside" name="q" value="<?=$arResult["REQUEST"]["QUERY"]?>" size="40" />
<?endif;?>
...
<?endif;?>
	&nbsp;<div class="button_gray">
		<input type="submit" value="<?=GetMessage("SEARCH_GO")?>" style="font: italic 14px/20px Georgia,tahoma;">
	</div>
...
<?elseif(count($arResult["SEARCH"])>0):?>
	...
	<?foreach($arResult["SEARCH"] as $arItem):?>
		<div class="element_search">
			<? if($arItem['MODULE_ID'] == 'iblock' && $arItem['PARAM1'] == '1c_catalog'): ?>
				<?
					$_iter = CIBlockElement::GetByID($arItem['ITEM_ID']);
					$el = $_iter->fetch();
					//echo "<pre>".var_export($el, true)."</pre>";
					$pic_id = 0;
					if($el['DETAIL_PICTURE']) $pic_id = $el['DETAIL_PICTURE'];
					elseif($el['PREVIEW_PICTURE']) $pic_id = $el['PREVIEW_PICTURE'];
					if($pic_id){
						//$_iter = CFile::GetByID($pic_id);
						//$pic = $_iter->fetch();
						//echo "<pre>".print_r($pic, true)."</pre>";
						$path = CFile::GetPath($pic_id);
	//					echo "!!!$path!!!";
						if(!empty($path)){
							
							$path = thumbnail($pic_id, 200, 150, false, TRUE);
							$path_big = CFile::GetPath($pic_id);
//							echo print_r($path).'<br>';
							
							echo "<div class=\"element_search_img\">"
								."<h3><a href=\"{$path_big}\" class=\"fancybox\"><img src=\"{$path}\" class=\"bdr\"></a></h3>"
							."</div>";
						}
					}
			...
			<?endif;?>
			<?/*div class="date">
				<div><?=GetMessage("SEARCH_MODIFIED")?> <?=$arItem["DATE_CHANGE"]?></div><?
				if($arItem["CHAIN_PATH"]):?>
					<div><?=GetMessage("SEARCH_PATH")?>&nbsp;<?=$arItem["CHAIN_PATH"]?></div><?
				endif;
				?>
			</div*/?>
		</div>

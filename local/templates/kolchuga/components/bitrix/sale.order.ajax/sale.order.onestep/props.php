		...
		foreach($arSource as $arProperties)
		{
			...
			<tr>
				...
				<td>
					...
					{
						?>
						<input class="input_inside" type="text" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>">
						<?
					}
					...
					{
						?>
						<textarea class="input_inside" rows="<?=$arProperties["SIZE2"]?>" cols="<?=$arProperties["SIZE1"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>"><?=$arProperties["VALUE"]?></textarea>
						<?
					}
					...
					?>
							...
							if(self == top){
								...
								setTimeout(function(){top.RestyleLocations();}, 100);
							</script>							
					<?
...
if(!empty($arResult["ORDER_PROP"]["USER_PROFILES"]))
{
	...
	<select name="PROFILE_ID" id="ID_PROFILE_ID" onChange="SetContact(this.value)" style="width:100px;">
	...
	<script type="text/javascript">
	if(self == top){
		function RestyleProfile(){
			$("#ID_PROFILE_ID").dropkick({
				change: function (value, label) {
					if(value == '(Новый профиль)') value = '';
					setTimeout('RestyleProfile();', 2000);
				}
			});
			$("#ID_PROFILE_ID").dropkick();
		}
	}
	setTimeout(function(){ top.RestyleProfile() }, 100);
	</script>
	<br />
	<br />
	<?

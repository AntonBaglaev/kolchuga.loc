...
<div class="rowanons col-sm-3 sidebar__menu pr-0 pl-0">
		  ...
		  ?>
		  <li <?=(empty($setSection) ? 'class="active"' : '')?>><a href="<?=$APPLICATION->GetCurPageParam("", array("sfilter","bfilter","UF_LINK","PAGEN_2","PAGEN_1"))?>">Все товары</a></li>
		  </ul>
		  <?$APPLICATION->ShowViewContent("block_brands_file");?>
		  <a name="productlist" id="productlist" style=" clear: both; display: block;"></a>
		  <? if(!empty($_REQUEST['bfilter']) || !empty($_REQUEST['sfilter']) || !empty($_REQUEST['PAGEN_2'])){
		  ...
		  <h3 class="separator"><?=(!empty($setSection) ? $setSection : 'Все товары бренда')?></h3>
		  <div class='sec_pag'>
		  <?
		  $tmp='withborder';

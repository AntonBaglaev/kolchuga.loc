...
foreach($arResult['ITEMS'] as $key => $arItem){
	...
	if($picture) {
		$resized = CFile::ResizeImageGet($picture, array('width' => 200, 'height' => 200), 'BX_RESIZE_IMAGE_PROPORTIONAL');
		$arResult['ITEMS'][$key]['PREVIEW_PICTURE']['SRC'] = $resized['src'];
	}
	if($arItem['PROPERTIES']['RAZMER']['VALUE']) {
		$size = $arItem['PROPERTIES']['RAZMER']['VALUE'];
		$l = -strlen($size);
		if (substr($arItem['NAME'], $l) == $size)
			$arResult['ITEMS'][$key]['NAME'] = trim(substr($arItem['NAME'], 0, $l));
	}

	if($arItem['PROPERTIES']['IDGLAVNOGO']['VALUE'] == $arItem['XML_ID']){
		$arResult['ITEMS'][$key]['TP'] = true;
	}
}

...
file_put_contents($_SERVER["DOCUMENT_ROOT"]."/upload/log/zaprosi_o_".date("dmY").".txt", "\n\n** ".date("d.m.Y H:i:s")." ****". __FILE__ ." ". __LINE__ ."**********\n".print_r($_REQUEST, true), FILE_APPEND | LOCK_EX);
	file_put_contents($_SERVER["DOCUMENT_ROOT"]."/upload/log/zaprosi_o_".date("dmY").".txt", "\n\n** ".date("d.m.Y H:i:s")." ***** ".print_r($contents, true), FILE_APPEND | LOCK_EX);
	echo $contents;
	die();
}
else

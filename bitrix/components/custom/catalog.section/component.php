...
if($this->startResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation, $pagerParameters)))
{
	...
	//добавление параметра страна в фильтр
	$explodedUrl = explode('/', $APPLICATION->GetCurDir());
	$newSeoh1 = '';
	
	if(strpos($explodedUrl[4], 'rujie')==true){

		$r = CIBLockElement::GetList (Array(), Array('XML_ID'=>$explodedUrl[4], 'ACTIVE'=>'Y', 'IBLOCK_ID' => '66'), false, false, Array('*'));
		while ($db_item = $r->GetNextElement())
		{
			$ar_res1 = $db_item->GetFields();
			if($ar_res1['NAME']!=''){
				$arFilter['PROPERTY_STRANA_VALUE'] = $ar_res1['NAME'];
				$ipropValues = new \Bitrix\Iblock\InheritedProperty\ElementValues(66,$ar_res1['ID']);
                $IPROPERTY  = $ipropValues->getValues();
				if($IPROPERTY["ELEMENT_PAGE_TITLE"]!=''){
                    global $newSeoh1;
					$newSeoh1 = $IPROPERTY["ELEMENT_PAGE_TITLE"];
				}
			}
		}	
...
if((strpos($explodedUrl[4], 'rujie')==true)and(count($arResult["ITEMS"])==0)){
...
global $newSeoh1;
if($newSeoh1!=''){
	$APPLICATION->SetTitle($newSeoh1);
    $APPLICATION->SetPageProperty("title", $newSeoh1.' - цена в интернет магазине Кольчуга');
}

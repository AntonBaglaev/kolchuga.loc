...
class WizardServices
{
	function GetTemplates($relativePath)
	{
		...
		while(($dirName = @readdir($handle)) !== false)
		{
			...
			if (file_exists($absolutePath."/".$dirName."/big_preview.jpg"))
				$arTemplate["SCREENSHOT"] = $relativePath."/".$dirName."/big_preview.jpg";
			else
				$arTemplate["SCREENSHOT"] = false;

			if (file_exists($absolutePath."/".$dirName."/small_preview.jpg"))
				$arTemplate["PREVIEW"] = $relativePath."/".$dirName."/small_preview.gif";
			else
		...
		if ($handle = @opendir($themePath))
		{
			while (($file = readdir($handle)) !== false)
			{
				...
				$arThemes[$file] = $arTemplate + Array(
					...
					"PREVIEW" => (file_exists($themePath."/".$file."/small_preview.jpg") ? $relativePath."/".$file."/small_preview.jpg" : false),
					"SCREENSHOT" => (file_exists($themePath."/".$file."/big_preview.jpg") ? $relativePath."/".$file."/big_preview.jpg" : false),
				);

...
if($iblockID == false)
{
	...
	$arFields = array(
		...
		"FIELDS" => array(
			...
			"SECTION_CODE" => array("IS_REQUIRED" => "Y", "DEFAULT_VALUE" => array("UNIQUE" => "Y", "TRANSLITERATION" => "Y", "TRANS_EAT" => "Y")),
		), 
		"CODE" => $iblockCode,
	...
	$result = CIBlockProperty::GetByID("PRICE", $iblockID);
	if($arResult = $result->GetNext())
	{
		$iproperty = new CIBlockProperty;
		$arFields = array(
			"HINT" => "Прайс-лист создается в инфоблоке \"Прайс-лист для услуг\". Раздел - это отдельный прайс-лист",
		);
		$iproperty->Update($arResult["ID"], $arFields);
	}
}
...
WizardServices::ReplaceMacrosRecursive(WIZARD_SITE_PATH, array("SERVICES_IBLOCK_ID" => $iblockID));
...
if($iblockID == false)
{
	...
	# Update property
	$result = CIBlockProperty::GetByID("REVIEWS", $iblockID);
	if($arResult = $result->GetNext())
	{
		$iproperty = new CIBlockProperty;
		$arFields = array(
			"HINT" => "Выберите отзыв, который будет показываться на детальной странице проекта",
		);
...
WizardServices::ReplaceMacrosRecursive(WIZARD_SITE_PATH, array("PROJECTS_IBLOCK_ID" => $iblockID));
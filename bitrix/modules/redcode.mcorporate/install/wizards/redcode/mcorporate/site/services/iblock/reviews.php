...
if($iblockID == false)
{
	...
	# Update property
	$result = CIBlockProperty::GetByID("SERVICE", $iblockID);
	if($arResult = $result->GetNext())
	{
		$iproperty = new CIBlockProperty;
		$arFields = array(
			"HINT" => "Название услуги будет показано на странице с отзывами",
		);
...
CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/search/index.php", array("REVIEWS_IBLOCK_ID" => $iblockID));
...
class Action extends TradingService\Reference\Action\DataAction
{
	...
	protected function loadExternalOrdersByList()
	{
		...
		{
			$parameters['status'] = TradingService\Beru\Status::STATUS_PROCESSING;
		}

		$this->externalOrders = TradingService\Beru\Model\OrderFacade::loadList($options, $parameters, $logger);
	}
	...
	protected function collectOrders()
	{
		...
		foreach ($this->externalOrders as $order)
		{
			...
			if (!$onlyPrintReady || $row['PRINT_READY'])
			{
				$rows[] = $row;
			}
		}
	...
	{
		/** @var \Yandex\Market\Trading\Service\Beru\Status $serviceStatuses */
	...
	{
		return $order instanceof TradingService\Beru\Model\Order
	...
	{
		if ($status === TradingService\Beru\Status::STATUS_PROCESSING && (string)$subStatus !== '')
		{

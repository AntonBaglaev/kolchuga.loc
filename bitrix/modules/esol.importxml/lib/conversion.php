	...
	public function ApplyExtraConversions($fKey, $val, $ID)
	{
		...
		if(count($arConv) > 0)
		{
			...
			if($this->isSection)
			{
				...
				$prefixPattern2 = '/(#(('.implode('|', $this->fieldSectPrefixes).')[A-Z0-9_]+)#|\$\{[\'"]#(('.implode('|', $this->fieldSectPrefixes).')[A-Z0-9_]+)#[\'"]\}|#DATETIME#)/';
			}
			else
			{
				$prefixPattern = '/^(PARENT_)?('.implode('|', $this->fieldPrefixes).')[A-Z0-9_]+$/';
				$prefixPattern2 = '/(#(PARENT_)?(('.implode('|', $this->fieldPrefixes).')[A-Z0-9_]+)#|\$\{[\'"]#(PARENT_)?(('.implode('|', $this->fieldPrefixes).')[A-Z0-9_]+)#[\'"]\}|#DATETIME#)/';
			}
			...
			foreach($arConv as $k=>$v)
			{
					...
					))
				{
					...
					elseif($v['THEN']=='MATH_SUBTRACT_PERCENT') $val = \Bitrix\EsolImportxml\Utils::GetFloatRoundVal($this->GetFloatVal($val) * (1 - $this->GetFloatVal($v['TO'])/100));

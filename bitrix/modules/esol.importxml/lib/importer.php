...
class Importer extends ImporterBase {
	...
	public function SetPropValues($arPropFields, $arProp=array())
	{
		...
		while($arPropVal = $this->GetNextPropVal($groupXpath, $groupName))
		{
			...
			{
				if($arProp['PROPERTY_TYPE']=='L') $this->GetListPropertyValue($arProp, $arFields);
				$val = $arFields['VALUE'];
			}
	...
	public function SaveSectionRecordAfter($sectionID, $arItem, $arItemFields=array())
	{
		...
		if($this->subSectionInSection)
		{
			...
			if($this->Xpath($this->currentXmlObj, $xpath2))
			{
				...
				{
					$this->SaveSectionRecord($arSubsectionItem, $sectionID, true);
	...
	public function SaveRecord($arItem, $sectionID=0)
	{
		...
		foreach($this->params['ELEMENT_UID'] as $tuid)
		{
			...
			elseif(strpos($tuid, 'IP_PROP')===0)
			{
				...
				else
				{
					if($propsDef[$uid]['PROPERTY_TYPE']=='S' && $propsDef[$uid]['USER_TYPE']=='directory')
					{
						$valUid = $this->GetHighloadBlockValue($propsDef[$uid], $valUid, false, true);
						$valUid2 = $this->GetHighloadBlockValue($propsDef[$uid], $valUid2, false, true);
						$canSubstring = false;
					}
					elseif($propsDef[$uid]['PROPERTY_TYPE']=='E')
					{
		...
		while($arElement = $dbRes->Fetch())
		{
			...
			$res = $this->SaveRecordAfter($ID, $IBLOCK_ID, $arItem, $arFieldsElement2);
			if($res==='timesup') return false;
			$duplicate = true; 
		}
		...
		if($allowCreate)
		{
			if($this->params['ONLY_UPDATE_MODE_PRODUCT']!='Y')
			{
				...
				if($ID = $this->AddElement($arFieldsElement))
				{
					...
					$res = $this->SaveRecordAfter($ID, $IBLOCK_ID, $arItem, $arFieldsElement);
					if($res==='timesup') return false;
				}
				else
				{
	...
	public function SaveRecordAfter($ID, $IBLOCK_ID, $arItem, $arFieldsElement=array())
	{
		...
		while($arProperty = $this->GetNextProperty($groupXpath, $groupName))
		{
				...
				if($isPropertyMap && array_key_exists('NAME', $arPropertyFieldsOrig))
				{
					...
					if(array_key_exists($fName, $propertyMap['MAP']))
					{
						...
						foreach($arFields as $fieldKey=>$field)
						{
							...
							if(!$isOffer && ($fgKey = $fName.'-'.$fieldKey) && in_array($fgKey, $this->fieldsForSkuGen))
							{
								//sku generate
							...
							else
							{
								...
								if($setNewOnly && !in_array($field, $this->fieldOnlyNew)) $this->fieldOnlyNew[] = substr($field, 6);
			...
			foreach($propIds as $propId)
			{
				...
				if(is_array($arProp) && isset($arProp['ID']))
				{
					...
					if($value!==false)
					{
						if(isset($arPropertyFields['VALUE_XML_ID']) && strlen($arPropertyFields['VALUE_XML_ID']) > 0)
						{
	...
	public function PrepareElementPictures(&$arFieldsElement, $IBLOCK_ID, $isOffer=false)
	{
		...
		foreach($arPictures as $picName)
		{
			if($arFieldsElement[$picName])
			{
				...
				$arFile = $this->GetFileArray($val, $arDef, $arFileParams);
	...
	public function SaveSKU($ID, $NAME, $IBLOCK_ID, $arItem, $convertedFields=array(), $arFParams=false)
	{
		...
		if(count($this->fieldsForSkuGen) > 0)
		{
		...
		$arUid = array();
		$currentUid = $this->params['ELEMENT_UID_SKU'];
		if(!is_array($currentUid)) $currentUid = array($currentUid);
		if($ID > 0 && !in_array('OFFER_IP_PROP'.$OFFERS_PROPERTY_ID, $currentUid)) $currentUid[] = 'OFFER_IP_PROP'.$OFFERS_PROPERTY_ID;
		...
		if($OFFER_ID && ($this->params['ELEMENT_NO_QUANTITY_DEACTIVATE']=='Y' || $this->params['ELEMENT_NO_PRICE_DEACTIVATE']=='Y' || ($this->params['ELEMENT_LOADING_ACTIVATE']=='Y' && !$ID)) && class_exists('\Bitrix\Catalog\ProductTable') && class_exists('\Bitrix\Catalog\PriceTable'))
		{
			foreach($arProductIds as $prodId)
			{
				...
				if(!empty($arOfferIds))
				{
					...
					if($active!==false)
					{
						$dbRes = \Bitrix\EsolImportxml\DataManager\IblockElementTable::GetListComp(array('ID'=>$prodId, 'CHECK_PERMISSIONS' => 'N'), array('ACTIVE'))->Fetch();
	...
	public function SaveProperties($ID, $IBLOCK_ID, $arProps, $needUpdate = false)
	{
		...
		foreach($arProps as $k=>$prop)
		{
			...
			if($propsDef[$k]['MULTIPLE']=='Y')
			{
				...
				{
					$arVal[$k2] = $this->GetPropValue($propsDef[$k], (is_string($val) ? trim($val) : $val), $IBLOCK_ID);
			...
			{
				$arProps[$k] = $this->GetPropValue($propsDef[$k], $prop, $IBLOCK_ID);
			}
						...
						if($propsDef[$pk]['MULTIPLE']=='Y')
						{
							if($saveOldVals)
							{
								...
								foreach($arProps[$pk] as $fpk2=>$fpv2)
								{
									...
									if(is_numeric($fileId)) $arProps[$pk][$fpk2] = \CFile::MakeFileArray($fileId);
	...
	public function GetPropValue($arProp, $val, $IBLOCK_ID=0)
	{
	...
	public function GetIblockElementValue($arProp, $val, $fsettings, $bAdd = false, $allowNF = false, $allowMultiple = false)
	{
		...
		if($relField && $relField!='IE_ID' && $arProp['LINK_IBLOCK_ID'])
		{
			...
			$dbRes = \Bitrix\EsolImportxml\DataManager\IblockElementTable::GetListComp($arFilter, array('ID'), array('ID'=>'ASC'), ($allowMultiple ? false : 1));
			...
			elseif($bAdd && $arFilter['NAME'] && $arFilter['IBLOCK_ID'])
			{
				...
				$el = new \CIblockElement();
				$val = $el->Add(array_merge($arFilter, $arElemFields), false, true, true);

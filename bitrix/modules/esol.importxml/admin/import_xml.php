				...
				elseif(isset($_POST['DATA_FILE']) && strlen($_POST['DATA_FILE']) > 0)
				{
					...
					if(strpos($_POST['DATA_FILE'], '/')===0) 
					{
						...
						if($filepath && file_exists($filepath) && $_POST['OLD_DATA_FILE'])
						{
							...
							$oldFileSize = ($_POST['OLD_FILE_SIZE'] > 0 ? $_POST['OLD_FILE_SIZE'] : (int)filesize($_SERVER['DOCUMENT_ROOT'].$arOldFile['SRC']));
		...
		<tr>
			...
			<td>
				<select name="SETTINGS_DEFAULT[IMAGES_CHECK_PARAMS]">
					<option value=""><?echo GetMessage("ESOL_IX_IMAGES_CHECK_DEFAULT"); ?></option>
					<option value="WO_NAME"<?if($SETTINGS_DEFAULT['IMAGES_CHECK_PARAMS']=='WO_NAME' || $SETTINGS_DEFAULT['ELEMENT_NOT_CHECK_NAME_IMAGES']=='Y'){echo ' selected';}?>><?echo GetMessage("ESOL_IX_IMAGES_CHECK_WO_NAME"); ?></option>
					<option value="WO_SIZE"<?if($SETTINGS_DEFAULT['IMAGES_CHECK_PARAMS']=='WO_SIZE'){echo ' selected';}?>><?echo GetMessage("ESOL_IX_IMAGES_CHECK_WO_SIZE"); ?></option>
					<option value="PATH_SIZES"<?if($SETTINGS_DEFAULT['IMAGES_CHECK_PARAMS']=='PATH_SIZES'){echo ' selected';}?>><?echo GetMessage("ESOL_IX_IMAGES_CHECK_PATH_SIZES"); ?></option>
					<option value="MD5"<?if($SETTINGS_DEFAULT['IMAGES_CHECK_PARAMS']=='MD5'){echo ' selected';}?>><?echo GetMessage("ESOL_IX_IMAGES_CHECK_MD5"); ?></option>
				</select>
			</td>
		</tr>
		 ...
		 <table width="100%"><tr><td width="50%">
			...
			<div class="detail_status" id="esol_ix_result_wrap">
				<?echo CAdminMessage::ShowMessage(array(
					...
					.'</div>'
					.($SETTINGS_DEFAULT['STAT_SAVE']=='Y' ? ('<b><a target="_blank" href="/bitrix/admin/'.$moduleFilePrefix.'_event_log.php?lang='.LANGUAGE_ID.'&find_profile_id='.($PROFILE_ID + 1).'&find_exec_id=" id="esol_ix_stat_profile_link">'.GetMessage("ESOL_IX_STATISTIC_LINK").'</a></b>') : ''),
					"HTML" => true,
				))?>
			</div>
		 </td></tr></table>

...
//		}
	...
	private static function CreateChildEvents($parentId, $arFields, $params, $changeFields)
	{
		...
		foreach($attendees as $userKey)
		{
				...
				&& ($arFields['CAL_TYPE'] != 'user' || $arFields['OWNER_ID'] != $attendeeId))
			{
					...
					&& ((!$params['fromWebservice']) || !empty($changeFields)))
				{
					...
					$meeting = unserialize($arFields['MEETING']);
					$sender = $userIndex[$childParams['arFields']['MEETING_HOST']];
					$sender['EMAIL'] = $meeting['MAIL_FROM'];
		...
		{
			\CAgent::addAgent(
				"CCalendarEvent::sendEventInvitationUsingIcal('" . serialize($arEventManagers) . "');",
				"calendar",
				"N",
				0,
				"",
			...
			foreach($deletedAttendees as $attendeeId)
			{
				...
				if ($att['EXTERNAL_AUTH_ID'] === 'email')
				{
					$arEventManagers[] = OutcomingEventManager::getInstance([
						'arFields' => $arFields,
						'icalMethod' => 'cancel',
						'userIndex' => $mailAttendeesIndex,
						'receiver' => $userIndex[$att['USER_ID']],
						'sender' => $currentAttendeesIndex[$att['MEETING_HOST']],
					]);
				}
			}
	...
	{
		$arEventManager = unserialize($arEventManagerInstances);
					...
					if ($entry['IS_MEETING'])
					{
						...
						foreach($childEvents as $chEvent)
						{
							...
							if (ICalUtil::isMailUser($chEvent['OWNER_ID']) && !$parentEvent)
							{
								...
								$arEventManager[] = OutcomingEventManager::getInstance([
									'arFields' => $chEvent,
									'icalMethod' => 'cancel',
									'userIndex' => $attendees,
						...
						{
							\CAgent::addAgent(
								"CCalendarEvent::sendEventInvitationUsingIcal('".serialize($arEventManager)."');",
								"calendar",
								"N",
								0,
								"",
			...
			if (!empty($ufIds) && \Bitrix\Main\Loader::includeModule('disk'))
			{
				foreach ($ufIds as $item)
				{
					...
					if ($type == FileUserType::TYPE_ALREADY_ATTACHED)
					{
						...
						if(!$attachedModel
							|| (!empty($fields['UF_WEBDAV_CAL_EVENT']) && !in_array($item, $fields['UF_WEBDAV_CAL_EVENT'])))
						{
							$isChangeFiles = true;
							continue;
						}

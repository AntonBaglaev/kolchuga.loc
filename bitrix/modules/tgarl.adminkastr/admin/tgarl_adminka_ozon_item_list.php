...
$arrSetting=\Bitrix\Main\Config\Option::getForModule('tg_ozon');
	...
	$arParams=[
		'URI'=>'/v1/product/list',
		'BODY'=>["page"=>$pageN,"page_size"=>$countItem],
	];
	$otvet=$obOzon->sendOzon($arParams);
...
if(!empty($_REQUEST['save'])){
	...
	foreach($ozonItem as $rl=>$val){
		if(!empty($setArray[$val['product_id']])){
			...
			/* $dbEl = \CIBlockElement::GetList(array('sort' => 'asc'), array('IBLOCK_ID' => 76, 'PROPERTY_offer_id'=>$val['offer_id']), false, false, Array('ID'));
			if($dbEl->SelectedRowsCount()>0){
				while($obEl = $dbEl->GetNext()){
					$what['ozon_id']=$val['product_id'];
				...
				$arLoadProductArray = Array(
				 ...
				 \Kolchuga\Settings::xmp($val,0, __FILE__.": ".__LINE__,true);die; 
			} */
		}
	}	
}	

...
class CPickpoint extends CAllPickpoint
{
    ...
    public function OnSCOrderOneStepDeliveryHandler(&$arResult, &$arUserResult)
    {
		...
		$_REQUEST['PP_DELIVERY_MIN'] = strlen($_REQUEST['PP_DELIVERY_MIN']) > 0 ? $_REQUEST['PP_DELIVERY_MIN'] : $_SESSION['PICKPOINT']['PP_DELIVERY_MIN'];
		$_REQUEST['PP_DELIVERY_MAX'] = strlen($_REQUEST['PP_DELIVERY_MAX']) > 0 ? $_REQUEST['PP_DELIVERY_MAX'] : $_SESSION['PICKPOINT']['PP_DELIVERY_MAX'];			
		...
		{ 
            ...
            foreach ($arResult['DELIVERY'] as &$arDelivery) 
			{
                ...
                if (strpos($arDeliveryCode['CODE'], 'pickpoint') !== false) 
				{
					...
					if ($arDelivery['PROFILES']['postamat']['CHECKED'] == 'Y') 
					{
    ...
    public static function addPickpointJs()
    {
		...
		if (!CJSCore::IsExtRegistered("pickpoint_deliveryservice"))
			CJSCore::RegisterExt("pickpoint_deliveryservice", array(	
